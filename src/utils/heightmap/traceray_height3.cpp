...
public:
    ...
    Spectrum Sample(int w, int w_ds, const Shape* heightfield, const Shape* heightfield_ds, Ray &ray, Sampler* sampler, int &depth,
		const BSDF* bsdf, const int constrain) {
		int h_ds = w_ds;
		int h = w;
		int downscale = h / h_ds;
        ...
        while (depth < MAX_DEPTH || MAX_DEPTH < 0) {
			...
			if (constrain > -1) {
				...
				if (std::abs(std::ceil(uvIdx.x / downscale) - uvIdx_tmp.x) > (constrain + Epsilon) ||
					std::abs(std::ceil(uvIdx.y / downscale) - uvIdx_tmp.y) > (constrain + Epsilon)) {
					break;
				}
				else {
					return Spectrum(0.0f);
		...
		if (rayIntersect(ray, its_tmp2, heightfield_ds)) {
			return Spectrum(0.0f);
		}
	...
	Spectrum Eval(int w, int w_ds, const Shape* heightfield, const Shape* heightfield_ds, Ray &ray, Sampler *sampler, const Vector wo_query, int &depth,
		...
		while (depth < MAX_DEPTH || MAX_DEPTH < 0) {
			...
			if (!rayIntersect(ray_nee, its_nee, heightfield)) {
				...
				Point2 uv_nee = its_nee.uv;
				Point2 uvIdx_nee;
				getUVIdx(uv_nee, uvIdx_nee, w, h);
				if (std::abs(std::ceil(uvIdx_nee.x / downscale) - uvIdx_tmp.x) > (constrain + Epsilon) ||
					std::abs(std::ceil(uvIdx_nee.y / downscale) - uvIdx_tmp.y) > (constrain + Epsilon)) {
					BSDFSamplingRecord bRec_Direct(its, its.toLocal(wo_query));
					bRec_Direct.sampler = sampler;
					Li += throughput * bsdf->eval(bRec_Direct);
					break;
   ...
   //         ray = Ray(its.p, wo, 0.0);
		...
		Ray ray_nee = Ray(its.p, wo_query, 0.0);
		Intersection its_tmp2;
		if (rayIntersect(ray_nee, its_tmp2, heightfield_ds)) {
			return Spectrum(0.0f);
		}
    ...
    int run(int argc, char **argv) {
		...
		const Point p = Point(static_cast<Float>(std::atof(argv[8])),
			static_cast<Float>(std::atof(argv[9])),
			static_cast<Float>(std::atof(argv[10])));
		const Vector wi = normalize(Vector(static_cast<Float>(std::atof(argv[11])),
			static_cast<Float>(std::atof(argv[12])),
			static_cast<Float>(std::atof(argv[13]))));
		...
		surfaceProps.setSpectrum("reflectance", Spectrum(0.9f));
		...
		const int constrain = std::atoi(argv[14]);
