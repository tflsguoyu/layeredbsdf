...
public:
	...
	int run(int argc, char **argv) {
		...
		const Vector wi = normalize(Vector(-0.70710678, 0, 0.70710678));
		const RayDifferential ray_init = RayDifferential(p + wi * 10.0f, -wi, 0.0f);

		const Vector wo = normalize(Vector(0, 0, 1));
		...
		Spectrum bsdfEvalVal, bsdfSampleVal;
		Float bsdfSamplePdf, bsdfEvalPdf;
...
#pragma omp parallel for
		...
		for (int omp_i = 0; omp_i < M; ++omp_i) {
			...
			surfaceBSDF->evalAndSample(bRec, bsdfEvalVal, bsdfEvalPdf, bsdfSampleVal, bsdfSamplePdf, samplers[tid]->next2D());;
			bsdfEval += bsdfEvalVal;
