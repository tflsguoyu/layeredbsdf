...
#define MICROFLAKE_AVG_COSINE_SAMPLE_COUNT 1000000
...
public:
    MicroflakePhaseFunction(const Properties &props) : PhaseFunction(props) {
        ...
        m_avgCosine = 0.0f;
        if ( props.getBoolean("computeAvgCosine", false) ) {
            ...
            mRec.orientation = Vector(0.0, 0.0, 1.0);
            PhaseFunctionSamplingRecord pRec(mRec, Vector(0.0, 0.0, 0.0));

            for ( long long i = 0; i < MICROFLAKE_AVG_COSINE_SAMPLE_COUNT; ++i ) {
                pRec.wi = -warp::squareToUniformSphere(sampler->next2D());
                sample(pRec, sampler);
                m_avgCosine += dot(-pRec.wi, pRec.wo);
            }
            m_avgCosine /= static_cast<Float>(MICROFLAKE_AVG_COSINE_SAMPLE_COUNT);

            Log(EInfo, "Estimated average cosine: %.2lf", m_avgCosine);
        }
    }
